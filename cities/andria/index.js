...
const getConstellationById = (searchId) => {
  return CONSTELLATIONS.find(({ id }) => id === searchId);
};

const getConstellationIndex = (searchId) => {
  return CONSTELLATIONS.findIndex(({ id }) => id === searchId);
};
...
const getConstellation = async (file, isHidden = true) => {
  const raw = readFileSync(resolve(`./constellations/${file}`)).toString();
...
app.listen(PORT, async () => {
  console.log(`Andria running at http://localhost:${PORT}`);
});
