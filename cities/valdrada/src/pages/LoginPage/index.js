...
export const LoginPage = () => {
  ...
  const [isShowingKeyboardGuide, setIsShowingKeyboardGuide] = useState(false);
  const [hasSeenKeyboardGuide, setHasSeenKeyboardGuide] = useLocalStorage(
    'seenValdradaKeyboardGuide',
    false
  );
  const [notes, setNotes] = useState([]);
  const [isShowKeyboard, setIsShowKeyboard] = useState(false);
  const [isFadingKeyboardGuide, setIsFadingKeyboardGuide] = useState(false);
  const { playAudio } = useAudioPlayer();
  const languageData = useLanguage();
  const { isEnglish } = languageData;

  useEffect(() => {
    if (isShowingKeyboardGuide) {
      setHasSeenKeyboardGuide(true);
      setTimeout(() => {
        setIsShowKeyboard(false);
      }, 2000);
      setTimeout(() => {
        setIsFadingKeyboardGuide(true);

        setTimeout(() => {
          setIsShowingKeyboardGuide(false);
        }, 5000);
      }, 30000);
    }
  }, [isShowingKeyboardGuide]);
    ...
    <LanguageContext.Provider value={languageData}>
      <Wrapper $isEnglish={isEnglish}>
        <HotKeys keyMap={keyMap} handlers={handlers}>
          ...
          >
            <Deus>
              ...
              {isShowKeyboard && (
                <Keyboard
                  delay={3}
                  $isFading={isShowingKeyboardGuide}
                  disabled={isPlayingSong || isAutoPlaying}
                  onClick={() => setIsShowingKeyboardGuide(true)}
                />
              )}
              <D
                onClick={dPlay}
          ...
          {isShowingKeyboardGuide && (
            <KeyboardGuide $isFading={isFadingKeyboardGuide}>
              <KeyboardIcon fill="#bda180" width="60" />
              a, b, c, d, e / 1, 2, 3, 4, 5
            </KeyboardGuide>
          )}
          <PlaySong>
